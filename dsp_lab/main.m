...
#  raw_audio_in = raw_audio_in';q
  ...
  bands = crossover(stereo_enh_out);
   
  band1 = 0.2 * dynamics_compress([bands(:,1),bands(:,2)], 5.0, -20.0, 3.0, 0.0001, 0.075);
  band2 = 0.2 * dynamics_compress([bands(:,3),bands(:,4)], 4.0, -17.0, 0.0, 0.00005, 0.075);
  band3 = 0.2 * dynamics_compress([bands(:,5),bands(:,6)], 4.0, -25.0, 0.0, 0.00002, 0.075);
  band4 = 0.2 * dynamics_compress([bands(:,7),bands(:,8)], 5.0, -25.0, 2.0, 0.000005, 0.025);

#  band1 = [bands(:,1),bands(:,2)];
#  band2 = [bands(:,3),bands(:,4)];
#  band3 = [bands(:,5),bands(:,6)];
#  band4 = [bands(:,7),bands(:,8)];
