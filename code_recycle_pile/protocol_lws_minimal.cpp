...
static CommandHandler* cmd_handler = CommandHandler::get_instance();
...
static int
callback_minimal(struct lws *wsi, enum lws_callback_reasons reason, // @suppress("Unused static function")
                        void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_RECEIVE:
			rx_msg = new char[len];
			memcpy(rx_msg, in, len * sizeof(char));
			lwsl_user("LWS_CALLBACK_RECEIVE - length %d\n", len);
			cmd_handler->publish_web_cmd(rx_msg, len);
			delete[] rx_msg;
